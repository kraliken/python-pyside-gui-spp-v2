...
class BankInternalCodeEditView(QWidget):
    def __init__(self):
        ...
        self.title_label = QLabel("Bank belső kód")
        ...
        # ===== JOBB OLDAL =====
        self.title_label = QLabel("Bank belső kód törzs lekérdezése")
    ...
    def load_xlsx_files(self):
        ...
        try:
            ...
            self.df["Belsokod"] = self.df["Belsokod"].astype("string")
            self.df["FokonyvText"] = self.df["FokonyvText"].astype("string")

            # Főkönyvi szám: float → int → string, NaN esetén "NA"
            self.df["Fokony"] = self.df["Fokony"].apply(
            ...
            header.resizeSection(0, 200)  # 0. oszlop = 250 px
    ...
    def run_database_save(self):
        ...
        # Ellenőrzés: minden cella nem üres és sztring típus
        if self.df.isnull().values.any():
            QMessageBox.warning(self, "Hiba", "Az adatmezők nem lehetnek üresek.")
            self.progress_dialog.accept()
            self.progress_dialog = None
            return

        success, message = self.db.insert_bank_internal_code_rows_bulk(self.df)
